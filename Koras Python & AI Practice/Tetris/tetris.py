...
pyautogui.FAILSAFE = True # move mouse to upper left hand corner to stop training just in case
...
MAX_GAMES = 4000
explore_change = 0.15
weights = [5, 4, 12, .5, 4]  # Initial weight vector
# weights = [0, 0, -10, 0, 0]


def run_game(weights, explore_change, games_counter):
    ...
    while True:  # game loop
        ...
        if games_counter%1 == 0:
            show_game(board, score, level, current_move, next_piece, falling_piece)

def show_game(board, score, level, current_move, next_piece, falling_piece):
    # drawing everything on the screen
    DISPLAYSURF.fill(BGCOLOR)
    draw_board(board)
    draw_status(score, level, current_move)
...
def get_parameters(board):
    ...
    for i in range(0, BOARDHEIGHT):
        ...
        for j in range(0, BOARDWIDTH):
            if int(board[j][i]) > 0:
                isFilled += 1
        filler[i] = isFilled / BOARDWIDTH #for each row adds the ratio of blocks to empty space to the filler array
        if(isFilled >= BOARDWIDTH): #overrides if this move gets a point
            override = True
    blockRatio = 0
    addedCount = 0
    for x in range(0, len(filler)):
        if filler[x] > 0:
            blockRatio += filler[x]
            addedCount += 1
    blockRatio /= addedCount #puts the average of all of filler's ratios over 1 block into a variable
    blockRatio *= 100
    if override:
        blockRatio = 1000
    ...
    return height_sum, diff_sum, max_height, holes, blockRatio
...
def get_expected_score(test_board, weights):
    ...
    height_sum, diff_sum, max_height, holes, blockRatio = get_parameters(test_board)
    # Height_sum is sum of the heights of each column combined. ---> want this to be smaller ~1/2 board size (o: 8-80 *A)
    # diff_sum is the sum of all of the differences in height between columns. ----> we want this to be small (o: 3-40 *B)
    # max_height is the height of our tallest column.  ---> we really really want this small (o: 2-20 *C)
    # holes is the number of empty spaces surrounded by blocks. ---> we also dont like holes (o: 0-50*D)
    # block ratio takes the average ratio of blocks to empty space of the most dense areas on the field (o: 4-30 *E)
    A = weights[0]
    B = weights[1]
    C = weights[2]
    D = weights[3]
    E = weights[4]
    test_score = float(A*-1*height_sum - B*diff_sum - C*max_height - D*holes/10 + E*blockRatio)
    # print("Height sum: " + str(height_sum) + "  Diff sum: " + str(diff_sum) + "  Max Height*: " + str(
    #     max_height) + "  Holes: " + str(holes) + "  Block ratio: " + str(int(blockRatio)) + "% ...  total: " + str(int(test_score)))
    return height_sum, diff_sum, max_height, holes, blockRatio, test_score
...
def find_best_move(board, piece, weights, explore_change):
    ...
    for rot in range(0, len(PIECES[piece['shape']])):
        for sideways in range(-5, 6):
            ...
            if test_board is not None:
                ...
                height_sum, diff_sum, max_height, holes, blockRatio, test_score = get_expected_score(test_board[0], weights)
    ...
    if random.random() < explore_change:
        ...
        move = best_move
        print("learned move (score): " + str(int(best_score)))
        # print("Height sum: " + str(height_sum) + "  Diff sum: " + str(diff_sum) + "  Max Height*: " + str(max_height) + "  Holes: " + str(holes) + "  Block ratio: " + str(blockRatio))
        print("")
    return move
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption('Tetr/io')

    show_text_screen('Tetr/io')
    games_completed = 0
    scoreArray = []
    weight0Array = []
    weight1Array = []
    ...
    while True:  # game loop
        games_completed += 1
        game_count += 1
        newScore, weights, explore_change = run_game(weights, explore_change, game_count)
        print("Game Number ", games_completed, " achieved a score of: ", newScore, "  ************************************")
        print("")
        # if(newScore >= 0): #just in case
        #     MAX_GAMES -= 100
        scoreArray.append(newScore)
        game_index_array.append(games_completed)
        weight0Array.append(-weights[0]) # WEIGHTS **
        ...
        if games_completed >= MAX_GAMES:
            ...
            # plt.plot(game_index_array, weight4Array, label="Average Higher Density")
